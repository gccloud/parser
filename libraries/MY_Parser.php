 ...
 * @version     20151113
 */
class CI_MY_Parser extends CI_Parser {
    /**
    ...
    protected function _parse($template, $data, $return = FALSE) {
        ...
        foreach($replace as $from => $to) {
            $template = str_ireplace($from, ( empty($to)==false ? $to : '%EMPTY_VAR%'), $template);
        }
        ...
        foreach($matches as $match) {
            ...
            foreach($data as $pos => $row) {
                ...
                foreach($row as $key => $val) {
                    ...
                    $temp[$this->l_delim.$key.$this->r_delim] = (isset($val)===true ? $val : '%EMPTY_VAR%');
                }
        ...
        if( ! empty($conditionals)) {
            foreach($conditionals as $conditional) {
                ...
                // No compouned statement found, we're making a direct boolean check
                    ...
                    $output = ( empty($statement)=== false && $statement != '0' && $statement != '%EMPTY_VAR%') ? $output : '';
                }
